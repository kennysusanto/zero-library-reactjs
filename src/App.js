...
function App() {
    ...
    let content;
    if (view === "books") {
        content = <Books localView={view} setLocalView={setView} setSelectedBook={setSelectedBook} viewType={viewType} setViewType={setViewType} />;
    ...
    let nav = (
        <Container id="nav">
            <ButtonGroup size="lg" className="mb-2 w-100">
                <Button variant={booksPages.includes(view) ? "primary" : "light"} onClick={(e) => navClick(e.target, "books")}>
                    <BsCollection></BsCollection> Books
                </Button>
                <Button variant={view === "news" ? "primary" : "light"} onClick={(e) => navClick(e.target, "news")}>
                    <BsNewspaper></BsNewspaper> News
                </Button>
                <Button variant={view === "profile" ? "primary" : "light"} onClick={(e) => navClick(e.target, "profile")}>
